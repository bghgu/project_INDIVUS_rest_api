...
router.post('/', async(req, res, next) => {
    ...
    const checkLikes = 'select * from PostLikes where ID = ? and post_id = ?';
    //토큰 검증이 성공할 경우
    if(ID != -1) {
        ...
        if(result.length != 0) {
            ...
            let is_like_clicked = await db.execute3(checkLikes, ID, postId);
            if(count != undefined) {
                ...
                res.status(201).send({
                    ...
                    is_like_clicked : is_like_clicked.length != 0 ? 1 : 0
                });
            }
        }else {
            res.status(401).send({
